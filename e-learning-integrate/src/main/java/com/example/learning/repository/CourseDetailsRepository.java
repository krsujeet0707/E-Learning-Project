...
public interface CourseDetailsRepository extends JpaRepository<CourseDetails, Integer> {
	...
	@Query(value="select services.serviceId,services.serviceName from courses_table join services ON services.serviceId=courses_table.serviceId  where courses_table.vendor_id=?1  group by services.serviceId,services.serviceName;",nativeQuery=true)
